...
同一篇论文在多份文件里重复时，保留被引数更高（也就是更新）的那条，字段更全的那条优先，`extra` 里的 `bibtex_key` 不会丢，`follow_depth` 取最浅的一层。判断「是不是同一篇」的规则只有一处实现（`cluster_id`，没有就用题名+链接），抓取时的去重、读回已有文件时的去重、以及合并与建图用的都是它——否则同一批文件用爬虫写和用 digest 合并会给出不同的重复数。

输入是普通的 JSONL：人会手改，别的脚本也会生成。所以每个要参与排序、求和、比较的字段都在**读入时**过一遍：`"2019"` 这种写成字符串的数字会被读成数字，写成别的类型（`"soon"`、列表、字典）就当作「Scholar 没给这个字段」，作者列表 `["A","B"]` 会拼成一行 byline。只有一个字段坏掉不该让整行记录（连题名和链接）一起报废，更不该让 digest 以 `TypeError` 结束——年份里混进一个字符串，排序就会当场炸掉。这类记录会被计数并在 `[in]` 那行报出来（`--json` 里是 `counts.repaired_records`），因为悄悄改数据比报错更糟。
...
分组时会做两处归一化，否则同一个去处会被拆散：所有 arXiv 预印本归为 `arXiv preprint`（Scholar 会把 arXiv 编号写进 venue），作者主页那种 `nature 521 (7553), 436-444, 2015` 会去掉卷号页码归为 `nature`；大小写不同也算同一组（显示时保留先出现的写法）。**分桶只有一处实现**：终端概览里的「出现最多的期刊」、Markdown 报告开头的「几个期刊、几位第一作者」、以及按期刊/作者分组的表格，用的是同一份桶——否则同一份报告会自己和自己对不上（开头说 2 个期刊，表格里只有 1 行）。
...
```
scholar_crawler/
  models.py     记录与请求的数据结构（检索请求、结果、作者主页、页结果）；目标名的两种写法：命令行旁边读的短名、脱离上下文读的全名；「是不是同一篇」的唯一规则，以及读回记录时的字段类型判断
